...
class InstanciaRecorridoMixto:
    ...
    def leer_datos(self,filename):
        ...
        print("exclusivos: ", self.exclusivos)
...
def agregar_restricciones(prob, instancia):
    ...
    # restriccion deseable 2: p_i = 1 (i en exclusivos)
    for i in range(n):
        if i in instancia.exclusivos:
            prob.linear_constraints.add(
                lin_expr=[cplex.SparsePair(ind=[f"p_{i}"], val=[1])],
                senses=["E"],
                rhs=[1]
            )

def armar_lp_tsp_sol_d2(prob, instancia):
...
def resolver_lp_tsp_sol_d2(prob, prob_tsp_mtz=None, config_number=0, mip_start=False):
...
def main():
    ...
    armar_lp_tsp_sol_d2(prob,instancia)

    # Resolucion del modelo
    resolver_lp_tsp_sol_d2(prob)
