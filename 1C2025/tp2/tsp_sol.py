...
def agregar_restricciones(prob, instancia):
    ...
    for i in range(1, n):
        prob.linear_constraints.add(
            lin_expr=[cplex.SparsePair(ind=[f"u_{i}", f"p_{i}"], val=[1, (-n+2)])],
            senses=["L"],
            rhs=[1]
        )
...
def armar_lp_tsp_sol(prob, instancia):
...
def resolver_lp_tsp_sol(prob, prob_tsp_mtz=None, config_number=0, mip_start=False):
...
def main():
    ...
    armar_lp_tsp_sol(prob,instancia)

    # Resolucion del modelo
    resolver_lp_tsp_sol(prob)
